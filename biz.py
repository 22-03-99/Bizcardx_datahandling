    ...
    if img is not None:
      ...
      if button_1:
        ...
        for index, row in concat_df.iterrows():
          ...
          # Query to check if the values already exist
          query = "select count(*) from bizcard_details where contact = ? and image = ?"  # Adjust column names and table name
          values = (row['CONTACT'],row['IMAGE'])  # Use user inputs in the query

          cursor.execute(query , values)
          count = cursor.fetchone()[0]

          if count > 0:
            st.success("Values already exist in the table. Insertion skipped.")
